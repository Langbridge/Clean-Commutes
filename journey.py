...
class RouteFactory():
    ...
    def __init__(self, start=None, end=None, bg_exposure=1, time=datetime.now(), user={"sex": 'M', "age": 30, "mass": 70}, deadline=None):
        ...
        self.user = self.format_user(user)
    ...
    def parse_routes(self,r):
        ...
        for i, route in enumerate(r['journeys']):
            ...
            x = Route(duration, arrive_time, fare, modes, legs, self.bg_exposure, self.user)
            routes[i] = x
    ...
    def format_user(self, user):
        if user['sex'].lower()=='male' or user['sex'].lower()=='m':
            user['sex'] = 'male'
        else: user['sex'] = 'female'

        if int(user['age']) < 31: user['age'] = '<31'
        elif int(user['age']) < 41: user['age'] = '31-<41'
        elif int(user['age']) < 51: user['age'] = '41-<51'
        else: user['age'] = '>51'
...
class Route():
    ...
    def __init__(self, duration, arrive_time, fare, modes, legs, bg_exposure, user):        
    ...
    def follow(self):
        journey_str = f"Leave {self.legs[0][0]} by {str(self.depart_time.strftime('%X'))}."
        for leg in self.legs:
            journey_str += f"{leg[1]} until you arrive at {leg[2]}."

    # CALCULATE THE RDD OF THE ROUTE FOR A GIVEN USER
    def calc_rdd(self, background_level):
        journey = {}
        ...
        exposure = RDD(self.user, journey)
